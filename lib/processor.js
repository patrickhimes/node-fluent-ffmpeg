...
var PRIORITY = require('./base-priority')
...
module.exports = function (proto) {
  ...
  proto._spawnFfmpeg = function (args, options, processCB, endCB) {
    ...
    this._getFfmpegPath(function (err, command) {
      ...
      if (utils.isWindows) {
        var priority = PRIORITY.BELOWNORMAL

        var pid = ffmpegProc.pid
        var renice = spawn('wmic', ['process','where',`handle=${pid}`,'call','setpriority','16384'], { windowsHide: true })
      }
  ...
  proto.renice = function (niceness) {
    if (!utils.isWindows) {
      ...
      this.options.priority = PRIORITY.BELOWNORMAL
  ...
  /**
   * Basepriority current and/or future ffmpeg processes
   *
   * Ignored on all platforms other than Windows.
   *
   * @method FfmpegCommand#basepriority
   * @category Processing
   *
   * @param {Number} [priority=0] niceness value of
   * @return FfmpegCommand
   */
  proto.basepriority = function (priority) {
    if (utils.isWindows) {
      priority = priority || PRIORITY.BELOWNORMAL
